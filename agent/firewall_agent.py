...
def register_with_server():
    ...
    sio.emit('register', registration_data)

@sio.on('policy_update')
def on_policy_update(data):
    """Handle policy updates from the server."""
    print(f"Received policy update: {data}")
    # Apply the policy on this endpoint
...
def load_policies():
    ...
    try:
        ...
        logging.error(f"Error loading policies from {policy_file}: {e}")
        return {
            "whitelist": {"applications": [], "domains": [], "ips": []},
            "blacklist": {"applications": [], "domains": [], "ips": []},
        }

# Initialize the policies from the JSON file
